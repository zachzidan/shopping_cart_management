...
<?php
	...
	$query = "SELECT Category.cat_id, Category.cat_name AS Category, Category.cat_desc AS Description, Category.cat_disp_cmd";
	# Append to the $query string - note the required space
	$query .= " FROM Category, CgryRel";
	$cat_id = $store_root_category;
	# If a category id has been provided, set the category id for WHERE clause
    if (isset($_GET["cat_id"])) {
    	$cat_id = $_GET["cat_id"];
    }
	$query .= " WHERE Category.cat_id = CgryRel.cgryrel_id_child AND CgryRel.cgryrel_id_parent = $cat_id";
...
<?php
    ...
    while($row = $statement->fetch(PDO::FETCH_ASSOC)) { ?> <!-- see http://www.php.net/manual/en/pdostatement.fetch.php -->
		<TR>
			...
			<TD><a href="<?php echo $row['cat_disp_cmd'].'?cat_id='.$row['cat_id'];?>"><?php echo $row['Category'];?></a></TD>
